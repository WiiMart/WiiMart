...
updateSQL = "SELECT * FROM gifted_titles WHERE towhom = ?";
pst = conn.prepareStatement(updateSQL);
pst.setString(1, request.getParameter("myFriendCode"));
ResultSet rs1 = pst.executeQuery();
while (rs1.next()) {
    gifts.add(new Gift(rs1.getString("titleid"), rs1.getString("fromwho"), rs1.getString("towhom")));
}
//END GIFTED TITLES STUFF
conn.close();
if (ogMode) {
...
function OnSetTask()
{
    try {
		...
		var strTaskUrl = "https://oss-auth.a.taur.cloud/BNR.bin";
...
{<%
	...
	var gifts = [];
	gifts[<%= i %>] = {
		titleId: "<%= gifts.get(i).titleId %>",
		from: "<%= gifts.get(i).from %>",
		to: "<%= gifts.get(i).to %>"
	}
...
<% } else { %>
Checking if you have new gifts...
<% } %>
<div id="Progress">
