...
class Gift {
    ...
    boolean acc = false;
    boolean rej = false;
	public Gift(String id, String fromWho, String toWhom, boolean accepted, boolean rejected) {
		this.titleId = id;
		this.from = fromWho;
		this.to = toWhom;
		this.acc = accepted;
...
<sql:query dataSource="${snapshot}" var="gifts">
    SELECT * FROM gifted_titles WHERE towhom = '${friendcode}';
</sql:query>
...
function OnSetTask()
{
    try {
		...
		var strTaskUrl = "http://ccs.tmp.idkwh.ct8.pl/cgi-bin/banner.php"; // serves banner dynamically (like Mario Kart Wii's ghost.ashx)
...
function initPage()
{
	...
	ec.setSessionValue("giftListSize", String(gifts.length));
...
<body onload="initPage();trace('launch code: ' + shop.launchCode);shop.disableHRP();">
