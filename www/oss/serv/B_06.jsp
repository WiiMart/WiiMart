...
//String[] Region_US = ["US"];
//String[] Region_KR = ["KR"];
%><%--TESTING FOR ALL B_06 PAGES--%><%
String country = request.getParameter("country") == null ? "US" : request.getParameter("country"); // COUNTY
String language = request.getParameter("language") == null ? "en" : request.getParameter("language");
String id = request.getParameter("titleId") == null ? "_" : request.getParameter("titleId");
String path = "";
if (id.equals("_")) path = "/root/www/MoreDetails/_.jsp";
else if (language.equals("jp")) path = "/root/www/MoreDetails/" + id + "_" + country + ".jsp";
else path = "/root/www/MoreDetails/" + id + "_" + country + "_" + language + ".jsp";
File file = new File(path);
String details = "";
if (!file.exists()) {
	details = new String(Files.readAllBytes(Paths.get("/root/www/MoreDetails/_.jsp")), "UTF-8");
} else {
	details = new String(Files.readAllBytes(Paths.get(path)), "UTF-8");
}
%><%= details %>
<%--<%
    ...
    Path path = Paths.get(application.getRealPath("/WEB-INF/classes/Details.json"));
    String json = new String(Files.readAllBytes(path), "UTF-8");
    JSONObject jsonobj = new JSONObject(json);
	JSONArray TitleDetailsArray = null;
    try {
		if (titleId.equals("000100014F484243")) { // exception for OHBC
			TitleDetailsArray = jsonobj.getJSONArray("Details_REGIONFREE");
		} else {
			TitleDetailsArray = jsonobj.getJSONArray("Details_" + region.toUpperCase() + "_" + language.toUpperCase());
		}
	} catch (org.json.JSONException e) {
		TitleDetailsArray = jsonobj.getJSONArray("Details_USA_EN");
	}

	String titleMagic = "";
    for (int i = 0; i < TitleDetailsArray.length(); i++) {
        JSONObject detailsarray = TitleDetailsArray.getJSONObject(i);
        String JSONTitleId = detailsarray.getString("TitleId");
        if (titleId.equals(JSONTitleId)) {

			if (detailsarray.has("DetailsBody") && !detailsarray.isNull("DetailsBody")) {
				request.setAttribute("DetailsBody", detailsarray.getString("DetailsBody"));
			} else {
				request.setAttribute("DetailsBody", "");
			}
			
			if (detailsarray.has("Image1") && !detailsarray.isNull("Image1")) {
				request.setAttribute("Image01", detailsarray.getString("Image1"));
			} else {
				request.setAttribute("Image01", "");
			}

			if (detailsarray.has("Image2") && !detailsarray.isNull("Image2")) {
				request.setAttribute("Image02", detailsarray.getString("Image2"));
			} else {
				request.setAttribute("Image02", "");
			};
			if (detailsarray.has("manual") && !detailsarray.isNull("manual")) {
				request.setAttribute("manual", detailsarray.getString("manual"));
			} else {
				request.setAttribute("manual", "false");
			}

			if (detailsarray.has("magic") && !detailsarray.isNull("magic")) {
				request.setAttribute("magic", detailsarray.getString("magic"));
				titleMagic = detailsarray.getString("magic");
			} else { 
				request.setAttribute("magic", ""); 
			}
        }
    }
%><%
...
<html>
<head>
	<script>console.log("<%= TitleDetailsArray %>")</script>
  <!--  -----------------------------------------------------  -->
<!--  Copyright 2005-2014 Acer Cloud Technology, Inc.        -->
<!--  All Rights Reserved.                                   -->
<!--                                                         -->
...
<script type="text/javascript" language="JavaScript">
	function showManual2(titleId, magic) {
		showAbsPage("/oss/oss/ext/manual/"+titleId+"/"+magic+"/html/"+titleId+"/<%= region.toUpperCase() %>/<%= language.toLowerCase() %>/index.html");
	}
</script>
<script type="text/JavaScript">
<!--
function initPage()
{
	...
	<c:if test="${not empty magic && manual eq 'true'}">
	setUnderButtonR(true, "View the Operations Guide", "javascript:showManual2('<%= titleId %>', '<c:out value="${magic}" />')", "wiiSelectSound()");
	</c:if>
	<c:if test="${manual eq 'true' && empty magic}">
	setUnderButtonR(true, "View the Operations Guide", "javascript:showNotImplementedYetFromB_06('<%= titleId %>')", "wiiSelectSound()");
	</c:if>
...
<div id="goodsfree">
  <div id="text" class="buttonTextBlackM"><c:out value="${DetailsBody}" escapeXml="false" /></div>
  <c:if test="${not empty Image01}">
  <c:if test="${Image01 ne ''}">
  <div id="image02">
    <img src="/oss/ccs/<%= titleId %>/<c:out value="${Image01}" />" width="160" height="120" alt="Screen Shot 2"/>
    </div>
	</c:if>
	</c:if>
  ...
  <div id="image01">
    <img src="/oss/ccs/<%= titleId %>/<c:out value="${Image02}" />" width="160" height="120" alt="Screen Shot 3"/>
    </div>
	</c:if>
	</c:if>
</div>
...
<div style="overflow:hidden" nowrap id="TitleName1stline">
  <div align="center" class="headerBlueL">
	<span class="contentsBlueM"><% if (!title1.equals("")) { %><%= title1 %><% } %></span>
</div>
</div>
<% if (!title2.equals("")) { %>
<div style="overflow:hidden" nowrap="" id="TitleName2stline">
	<div align="center" class="headerBlueL">
		<span class="contentsBlueM"><%= title2 %></span>
	</div>
</div>
<% } %>
