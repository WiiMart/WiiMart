...
<a href="javascript:location.reload()"> reload</a><BR/><BR/><BR/><br/><BR/><BR/><BR/><BR/><BR/><BR>
<!--<span id="inf">itemId: <%= request.getParameter("itemId") %><BR/>recipient: <%= request.getParameter("recipient") %><BR/>recipientDeviceCode: <%= request.getParameter("recipientDeviceCode") %></span>-->
...
function initPageCommon()
{
	...
	ccsUrl = 'http://198.62.122.200/ccs/download'; // why is this an ip??
...
function purchaseGiftTitle(titleId, itemId, price, limits)
{
    ...
    if (titleId == null) {
        ...
        notes += '<Recipient><DeviceCode>' + recipient +'</DeviceCode></Recipient>';
		//Next two lines added by Ocarina on 6/24/25 (or 24/6/25 for those in EU)
		notes += '<Message>' + ec.getSessionValue("giftMessage") + "</Message>";
		notes += '<MiiNumber>' + ec.getSessionValue("selectedMiiIdx") + '</MiiNumber></GiftInfo>';
		/*var xml = new XMLHttpRequest();
		let giftUrl = "https://oss-auth.a.taur.cloud/oss/serv/giftTitle.jsp?titleId=" + titleId + "&from=" + nwc24.myUserId + "&to=" + recipient;
		xml.open("GET", giftUrl);
		xml.send();
		xml.onreadystatechange = function() {
			if (xml.status == 200) {
				onGiftDone({status: -200, errCode: 0})
			} else {
				onGiftDone({status: -xml.status, errCode: xml.status});
			}
		}*/
...
function doGifting()
{
	// prevent gift from being sent (can't use WC24/WL24)
...
function initPage()
{
	...
	setUnderButtonL(false, "Back", "javascript:history.back()", "wiiCancelSound()");
	...
	trace("set recipientName");

	confirmPurchaseYes();
	trace("finished initPage");
}
...
<!-- How is initPage failing? -->
<body onload="try { initPage(); }catch(err) {document.getElementById('text03-01').innerHTML = 'ERROR: '+err;}">
<!--  -----------------------------------------------------  -->
<!--  Copyright 2005-2014 Acer Cloud Technology, Inc.        -->
<!--  All Rights Reserved.                                   -->
<!--                                                         -->
...
<% if (title2 != "")  { %>
<div id="TitleName2stline" nowrap style="position:absolute; left:64px; top:113px; width:480px; overflow:hidden; z-index:34">
