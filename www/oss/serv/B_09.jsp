<%@ page import = "org.wiimart.Catalog,org.wiimart.Title,java.io.*,java.util.*,java.net.http.*,java.net.URI,java.net.http.HttpResponse.BodyHandlers,java.net.HttpURLConnection,java.net.URL,java.nio.charset.StandardCharsets,org.json.*,java.nio.file.*,java.io.*" %>
...
function showSDSelection(checkSDFlag)
{
    ...
    if (ecSupportsSession()) {
        ...
        if (status == "receiving") {
            setUnderButtonL(true, 'Later', "javascript:goGiftNext('<%= titleId %>')", "wiiCancelSound()");
...
function initPage()
{
	...
	if (ecSupportsSession()) {
		...
		if (status == "receiving") {
			setUnderButtonL(true, 'Now', "javascript:showPage('B_10.jsp?SD=<%= request.getParameter("SD") == null ? "N" : request.getParameter("SD") %>&titleId=<%= titleId %>')", "wiiSelectSound()");
			setUnderButtonR(true, 'Later', "javascript:goGiftNext('<%= titleId %>')", "wiiCancelSound()");
      ...
      titlePoints = "0";
			...
			setUnderButtonL(true, 'Yes', "javascript:showPage('B_10.jsp?SD=<%= request.getParameter("SD") == null ? "N" : request.getParameter("SD") %>&titleId=<%= titleId %>')", "wiiSelectSound()");
...
<div id="pointword02">
  <div align="right" class="contentsRedM"><script>document.write(Number(ec.getCachedBalance()) + Number("<%= points %>"))</script></div>
</div>
<div id="pointword03">
  <div align="right" class="catalogTitleBlack_01"></div>
</div>
