...
function P_02_setUnderButtons() {
	setUnderButtonL(true, "Quit", "javascript:showPoints()", "if(snd) snd.playSE(cSE_Cancel)");
	setUnderButtonR(true, "OK", "javascript:checkUserBalance()", "if(snd) snd.playSE(cSE_Decide)");
...
<script type="text/JavaScript">
	function checkUserBalance() {
		validate();/*
		trace(getBalance());
		var maxBalance = parseInt("20000");
		if (ec.getBalance() + 1000 > maxBalance) {
			setUnderButtonL(false);
			setUnderButtonR(false);
			setUnderButton(true, "Try Again", "javascript:retry('invalidNum')", "if(snd) snd.playSE(cSE_Cancel)");
			hideElement("text01-01");
			hideElement("text02-01");
			hideElement("pointcard");
			hideElement("inputs");
			showElement("invalidNum");
			document.getElementById("errorCodePlaceholder").innerHTML = '206668';
			document.getElementById("errorTextPlaceholder").innerHTML = 'Your Wii Points balance cannot exceed <span id="maxBalance">20000</span> Wii Points.<br /><br />You cannot redeem this Wii Points Card at this time. Adding these points will put you over the limit.';
		} else {
			validate();
		}*/
	}
...
<form id="cardForm" action="https://oss-auth.a.taur.cloud/oss/serv/P_04.jsp" method="post" name="cardForm">
