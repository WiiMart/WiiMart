...
<% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%><a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a>&nbsp;<a href="javascript:window.location.reload();">reload</a>&nbsp;<script>var x = new wiiShop();if (x.isCompatibleMode){document.write('hello vwii (cmptmode ret '+x.isCompatibleMode+')');}else{document.write('hello wii (cmptmode ret '+x.isCompatibleMode+')');}</script><%}%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="sql" uri="http://java.sun.com/jsp/jstl/sql" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
<%@ page buffer="8192kb" autoFlush="true" %>
<%@ page trimDirectiveWhitespaces="true"%>
<c:set var="friendCode" value="${param.deviceCode}" scope="page" />
...
<c:set var="giftsPending" value="false" scope="page" />
<c:if test="${gifts.rowCount gt 0}">
	<c:set var="giftsPending" value="true" />
</c:if>
<!--  -----------------------------------------------------  -->
...
function initPage()
{
    ...
    var isShoppingManualEnabled = "true";
        ...
        if (isShoppingManualEnabled == "true") {
        	setUnderButtonR(true, "Shopping Guide", "javascript:showAbsPage('/oss/oss/ext/manual/0001000248414241/FFFD1AAA/html/manual/NOA/startup.html')", "wiiSelectSound()");
    	} else {
...
<div id="text04-01">
  <table width="100%" height="100%"><tr><td align="center" valign="middle"><div align="center" class="buttonTextBlackM">
	<script>
		trace("og mode: " + ec.getSessionValue("ogMode"));
		if (ec.getSessionValue("ogMode") == "true") {
  			document.write("Account Activity");
		} else {
			document.write("Pending Gifts");
		}
	</script>
  </div></td></tr></table>
</div>
...
<div id="spacer04">
  ...
  <c:if test="${giftsPending eq true && param.og eq false}">
  	<img id="newGifts" src="/oss/oss/common/images//banner/NEW_en.gif">
  </c:if>
...
<div id="W_03-title" align="left" class="titleBlueL"><script>if (ec.getSessionValue("ogMode") == "true") { document.write(shop.title); } else { document.write("WiiMart"); };</script></div>
