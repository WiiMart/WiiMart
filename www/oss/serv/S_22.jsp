...
<%
	String rank = request.getParameter("r") == null ? "NONE_NONE" : request.getParameter("r");
	String[] ranks = {"SCA_GOLD_01", "CSS_SILVER_01", "SCA_GOLD_02", "CSS_SILVER_02", "NONE_NONE"};
	String[] icrRanks = {
		"SCA_GOLD", "CSS_SILVER", "NONE_NONE",
		"SCA_GOLD_TOICR", "CSS_SILVER_TOICR",
	};
%>
<!--S_22-->
...
<title>???S_22.title???</title>
	<link href="/oss/oss/common/css/S_22.css" rel="stylesheet" type="text/css" />
...
var icrDiscountType = '';
	...
	icrStatus = 'DISABLED';
    icrDiscountType = 'ICR1';
    trace("icrStatus: " + icrStatus);
    if(ecSupportsSession()){
//        ec.setSessionValue("icrStatus","ACTIVATED");
        ec.setSessionValue("icrStatus",""+icrStatus);
        ec.setSessionValue("icrDiscountType",""+icrDiscountType);
    }
...
function showSilver01() {
	showElement("Silver-01");
	setUnderButton(true, "OK", "javascript:showSilver02()", "wiiSelectSound()");
}
function showSilver02() {
	hideElement("Silver-01");
	showElement("Silver-02");
	setUnderButton(true, "OK", "javascript:submit()", "wiiSelectSound()");
}
function showSilverToIcr() {
	showElement("Silver-ToIcr");
	setUnderButton(true, "OK", "javascript:showPage('S_30.jsp')", "wiiSelectSound()");
}


function showGold01() {
	showElement("Gold-01");
	setUnderButton(true, "OK", "javascript:showGold02()", "wiiSelectSound()");
}
function showGold02() {
	hideElement("Gold-01");
	showElement("Gold-02");
...
function showGoldToIcr() {
	showElement("Gold-ToIcr");
	setUnderButton(true, "OK", "javascript:showPage('S_30.jsp')", "wiiSelectSound()");
}
...
function initPage() {
	...
	trace("S-22 from: " + previousPage);
	if((previousPage != null) && (previousPage.indexOf('S_18.jsp') >= 0)) {
		trace("S-22fromInit: false");
		ec.setSessionValue("S-22fromInit", "false");
	} else {
		trace("S-22fromInit: true");
		ec.setSessionValue("S-22fromInit", "true");
	}

	if(icrStatus == 'ACTIVATED') {
		if(goldStatus == 'ELIGIBLE') {
			showGoldToIcr();
			return;
		} else if(silverStatus == 'ELIGIBLE') {
			showSilverToIcr();
			return;
		}
	} else {
		if(goldStatus == 'ELIGIBLE') {
			document.getElementById("inputDiscountType").value = "SCA20";
			document.getElementById("inputPromoType").value    = goldPromoType;
			showGold01();
			return;
		} else if(silverStatus == 'ELIGIBLE') {
			document.getElementById("inputDiscountType").value = "SCA10";
			document.getElementById("inputPromoType").value    = silverPromoType;
			showSilver01();
			return;
		}
	}
...
<% if (rank.equals("CSS_SILVER_01") || rank.equals("CSS_SILVER_02")) { %>
<div id="Silver-01" class="page_area bg_connect">
	<div class="page_title_area titleBlackM">
	  Connection Ambassador authorization</div>
	<div class="page_content_area contentsBlackM">
	  <br />Congratulations! You&rsquo;ve helped 10 people!<BR><BR>You are now a Connection Ambassador 10.</div> 
</div>

<div id="Silver-02" class="page_area bg_connect">
	<div class="page_title_area titleBlackM">
	  Connection Ambassador authorization</div>
	<div class="page_content_area contentsBlackM">
	  As a bonus, all Connection Ambassador 10s can download the entire Nintendo library of NES titles free of charge.<BR><BR>Any of these titles that you have already downloaded may be gifted once to a Wii Friend, also free of charge.</div> 
</div>
<% } else if (rank.equals("CSS_SILVER_TOICR")) { %>
<div id="Silver-ToIcr" class="page_area bg_connect">
	<div class="page_title_area titleBlackM">
	  Connection Ambassador authorization</div>
	<div class="page_content_area contentsBlackM">
	  Congratulations! You&rsquo;ve helped 10 people!<BR><BR>Before you claim your Connection Ambassador 10 bonus, please first download a title from the One Free Title list.</div> 
</div>
<% } else if (rank.equals("SCA_GOLD_01") || rank.equals("SCA_GOLD_02")) { %>

<div id="Gold-01" class="page_area bg_sca">
	<div class="page_title_area titleBlackM">
	  Connection Ambassador authorization</div>
	<div class="page_content_area contentsBlackM">
	</div> 
...
<div id="Gold-02" class="page_area bg_sca">
	<div class="page_title_area titleBlackM">
	  Connection Ambassador authorization</div>
	<div class="page_content_area contentsBlackM">
	  As a bonus, all Connection Ambassador 20s can download the entire library of titles for the NES, SNES, and Nintendo 64 free of charge.<BR><BR>Any of these titles that you have already downloaded may be gifted once to a Wii Friend, also free of charge.</div> 
</div>
<% } else if (rank.equals("SCA_GOLD_TOICR")) { %>
<div id="Gold-ToIcr" class="page_area bg_sca">
	<div class="page_title_area titleBlackM">
	  Connection Ambassador authorization</div>
	<div class="page_content_area contentsBlackM">
	  Congratulations! You&rsquo;ve helped 20 people!<BR><BR>Before you claim your Connection Ambassador 20 bonus, please first download a title from the One Free Title list.</div> 
</div>


<form id="form" method="post" action="S_27.jsp" style="display: none">
	<div id="commonFields"></div>
	<input id="inputIsInitial" type="hidden" name="isInitial" value="" />
	<input id="inputPromoType" type="hidden" name="promoType" value="" />
	<input id="inputDiscountType" type="hidden" name="discountType" value="" />
</form>
<% } else { %>

<% } %>
</body>
<% } %><%--IF rank is NONE_NONE --%>
</html>