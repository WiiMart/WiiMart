...
<link rel="shortcut icon" href="/oss/favicon.png" /> 
...
function showControllerDetailsOrWarning()
{
	var history = ec.getSessionValue('history.-1');
	var nextUrl = "B_05.jsp?titleId=<%= request.getParameter("titleId") %>";
	if ((history != null) && (history != undefined)) {
		// pre friend selection, no need for friend parameters
		if (history.indexOf("B_28") >= 0 || history.indexOf("/B_28") >= 0) {
			nextUrl = "B_28.jsp?titleId=<%= request.getParameter("titleId") %>";
		} else if (history.indexOf("B_08") >= 0 || history.indexOf("STAFF%20ONLY/B_08") >= 0) {
			nextUrl = "B_08.jsp?titleId=<%= request.getParameter("titleId") %>";
		} else if (history.indexOf("B_29") >= 0 || history.indexOf("STAFF%ONLY/B_29") >= 0) {
			nextUrl = "B_29.jsp?titleId=<%= request.getParameter("titleId") %>";
		} else {
			nextUrl = "B_05.jsp?titleId=<%= request.getParameter("titleId") %>";
			//nextUrl = "B_05.jsp?titleId=<%= request.getParameter("titleId") %>"
		}
	}
	showPage(nextUrl);
...
function initPage()
{
    ...
    setUnderButtonL(true, 'Back', "javascript:showControllerDetailsOrWarning()", "wiiCancelSound()");
