...
<%@ page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %><% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%><a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a><% } %>
...
<div class="catalogTitleBlack" id="text02-01">
  <p id="wii_message" class="styleConfirm">The country setting for the console has changed since the last time you connected to <% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %>the Wii Shop Channel<% } %>.&nbsp;If you have pending gifts, you won&rsquo;t be able to receive them after changing your country setting.</p>
  <p id="wiiU_message" class="styleConfirm">The country setting for the console has changed since the last time you connected to <% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %>the Wii Shop Channel<% } %>.</p>
  <p class="styleConfirmCountryChange">
  	Previously:&nbsp;<c:if test="${not empty param.country}">
	<c:out value="${param.country}" />
	</c:if>
	<br>
  	Currently:&nbsp;<c:if test="${not empty param.origCountry}">
	<c:out value="${param.origCountry}" />
	</c:if></p>
  <p class="styleConfirmWithLoyalty">
     <span id="confirmWithLoyaltyMsg" style="display:none">
		A Club Nintendo Membership is currently linked to this <% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %>Wii Shop Channel<% } %> account, and can&rsquo;t be used with the current country setting. If you press OK, the link will be removed. Note: This removes the link but does not delete either your Club Nintendo account or your <% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %>Wii Shop Channel<% } %> account.<BR><BR>If you do not want to remove the link, press &quot;Wii Menu&quot; and set the country to the previous setting.</span>
