...
<%
	...
	if (serial.equals("")) { %>
		<script>top.location = "https://oss-auth.a.taur.cloud/oss/serv/index.jsp"</script>
	<% } else {
    ...
    String updateSQL = "SELECT og_title FROM public.userbase WHERE serial_number = ?";
	...
	String theBool = "";
	while (rs.next()) {
		theBool = rs.getString("og_title");
	}
	conn.close();
	
	currTitle = theBool;
	%><script>console.log("current title: <%= currTitle %>")</script><%
	}
%>
<!--  -----------------------------------------------------  -->
...
function initPage()
{
	...
	MM_preloadImages('/oss/oss/common/images//banner/card01_b.gif',
		'/oss/oss/common/images//banner/wiimart_b.png',
		'/oss/oss/common/images//banner/wsc_b.png',
	...
	setUnderButtonL(true, "Back", "javascript:showPage('S_01.jsp');", "wiiCancelSound();");
    r = ec.getDeviceInfo();
	if(r.language == 'ja'){
	...
	var currTitle = ec.getSessionValue("currTitle");
	if (currTitle == "WiiMart") {
		hideElement("card02Spacer");
		showElement("card02SpacerDisabled");
	} else if (currTitle == "Wii Shop Channel") {
		hideElement("card03Spacer");
		showElement("card03SpacerDisabled");
	}
...
function select(title)
{
		...
		form.action = getSecureUrl("changeTitle.jsp");
		form.id = "titleChangeForm";
		form.innerHTML = "";
		form.innerHTML += '<div id="commonFields"></div>';
		form.innerHTML += '<input type="hidden" name="title" value="' + title + '"/>';
...
<div id="card02" name="cardMC" >
    ...
    onmouseout="MM_swapImgRestore()" onclick="select('WiiMart')"
    onmouseover="MM_swapImage('card02Img','','/oss/oss/common/images//banner/wiimart_b.png',1); wiiFocusSound();"/>
    <img src='/oss/oss/common/images//banner/wiimart_a.png' class='cardImg' id="card02Img"/>
    <img id='card02Shadow' class='cardShadow' src='/oss/oss/common/images//banner/banner03_shadow.gif' />
    <div id="text04-01" class="cardText buttonTextBlackM">WiiMart</div>
	<img class="cardImg" style='display:none;' src='/oss/oss/common/images//banner/title_grey.png' border="0" id='card02SpacerDisabled'>
</div>

<div id="card03" name="cardVisa" >
    <img src='/oss/oss/common/images//spacer.gif' class='cardSpacer' id="card03Spacer" 
    onmouseout="MM_swapImgRestore()" onclick="select('WSC')"
    onmouseover="MM_swapImage('card03Img','','/oss/oss/common/images//banner/wsc_b.png',1); wiiFocusSound();"/>
    <img src='/oss/oss/common/images//banner/wsc_a.png' class='cardImg' id="card03Img" />
    <img id='card03Shadow' class='cardShadow' src='/oss/oss/common/images//banner/banner03_shadow.gif' />	
    <div id="text05-01" class="cardText buttonTextBlackM">Wii Shop Channel</div>
	<img class="cardImg" style='display:none;' src='/oss/oss/common/images//banner/title_grey.png' border="0" id='card03SpacerDisabled'>
</div>

<div id="text01-01" class="titleBlackL">WiiMart Title Change</div>
<div id="text02-01"  class="catalogTitleBlack_02" align="left">Please select the title you would like to see when launching WiiMart.
<br><br><center>The option that is disabled is the title that is currently selected.</center></div>
</body>
</html>
