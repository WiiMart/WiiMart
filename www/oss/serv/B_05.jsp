...
<%@ page import = "org.wiimart.Catalog,org.wiimart.Title,java.io.*,java.util.stream.Collectors,java.util.*,java.net.http.*,java.net.URI,java.net.http.HttpResponse.BodyHandlers,java.net.HttpURLConnection,java.net.URL,java.nio.charset.StandardCharsets,org.json.*,javax.servlet.*,org.json.*,java.nio.file.*,java.io.*" %>
<%@ page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %>
<% String og = request.getParameter("og") == null ? "false" : request.getParameter("og"); %>
<% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%><a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a> <a align="right" href="javascript:window.location.reload();">reload</a><% } %>
...
options.put("titleId", titleId);
if ((language.equals("jp") || language.equals("")) && region.equals("JPN")) language = "ja";
Title title = catalog.getTitle(options);
...
Map<String, String> localizedDates = Arrays.stream(new String[][] {
	{"en", "Released "}, {"es", "Año: "},
	{"fr", "Parution "}, {"de", "Erschienen: "}, {"it", "Pubblicato: "}, {"nl", "Uit: "},
	{"ja", "発売 "}, {"ko", "발매 "},
}).collect(Collectors.toMap(loc -> loc[0], loc -> loc[1]));

//Prefix for title dates
String ldtIndex = region.equals("JPN") ? "ja" : (region.equals("KOR") ? "ko" : language);
String datePrefix = Optional.ofNullable(localizedDates.get(ldtIndex.toLowerCase())).orElse("");

//Tmd size stuff
byte[] tmdBytes = Files.readAllBytes(Paths.get("/root/nus/ccs/download/" + titleId + "/tmd"));
long tmdSize = tmdBytes.length;
%>
<%
String id = title.titleId.replace("00010002", "00010001");
String thumbnail = title.thumbnail;
String title1 = title.title1;
String title2 = title.title2;
String platform = title.console;
if (platform.equals("WII")) {
    platform = "Wii Channels";
} else if (platform.equals("WIIWARE")) {
    platform = "WiiWare";
};
String oldDate = title.date;
String releaseDate = "";

if (titleId.equals("0001000148434A50")) { // Hide for BBC iPlayer
  releaseDate = title.date;
} else {
  releaseDate = datePrefix + title.date;
};

String genre = title.genre;
String publisher = title.publisher;
String points = title.points;
String players = title.players;
String ratingDetails = title.ratingDetails;
String rating = title.rating.toUpperCase();
String controllers = title.controllers;
String size = title.titleSize;
String latestVersion = "" + (tmdBytes[476] + tmdBytes[477] + tmdBytes[478]);

    // Wfc logo stuff
    /*Path path = Paths.get(application.getRealPath("/WEB-INF/classes/wfc.json"));
    String json = new String(Files.readAllBytes(path), "UTF-8");
    JSONObject jsonobj = new JSONObject(json);
	  JSONArray WFC_Array = jsonobj.getJSONArray("WFC");
    JSONArray AltWFC_Array = jsonobj.getJSONArray("wiimmfi");
    JSONArray WFCPAY_Array = jsonobj.getJSONArray("WFCPay");*/
    String WIFITYPE = "NONE";
    String wiimmfi = "false";
    
    /*for (int i = 0; i < AltWFC_Array.length(); i++) {
      if (titleId.equals(AltWFC_Array.getString(i))) {
        WIFITYPE = "WIIMMFI";
        if (og.equals("false")) {
          wiimmfi = "true";
        }
    ...
    if (wiimmfi.equals("false")) {
    for (int i = 0; i < WFC_Array.length(); i++) {
      if (titleId.equals(WFC_Array.getString(i))) {
        WIFITYPE = "WFC";
      }
    }

    for (int i = 0; i < WFCPAY_Array.length(); i++) {
      if (titleId.equals(WFCPAY_Array.getString(i))) {
        if ("WFC".equals(WIFITYPE)) {
          WIFITYPE = "NONE";
        } else {
          WIFITYPE = "WFCPAY";
        }
  ...
  request.setAttribute("OSS_WIFI_METHOD_TYPE", WIFITYPE);
%>
...
<script type="text/JavaScript">console.log("OSS_WIFI_METHOD_TYPE: <%= WIFITYPE %>")</script>
...
function initPageCommon()
{
	...
	ecsUrl = 'https://oss-auth.a.taur.cloud/oss/ecs/services/ECommerceSOAP';

	iasUrl = 'https://oss-auth.a.taur.cloud/oss/ias/services/IdentityAuthenticationSOAP';

	ccsUrl = 'https://oss-auth.a.taur.cloud/oss/ccs/download';

	ucsUrl = 'https://oss-auth.a.taur.cloud/oss/ccs/download';
...
<title>Details</title>
  <link href="/oss/oss/common/css/B_05.css?fsfffsfdwwsfwwds" rel="stylesheet" type="text/css" />
...
function showSDSelectionAfterGift()
{
    ec.setSessionValue("giftStatus", "receiving");
    hideElement("giftText");
    document.getElementById("heading").innerHTML = 'Download Location';
    hideElement("details");
    hideElement("sdErrorDiv");
    hideElement("sdErrorNoSizeDiv");
    ...
    showBuyButtons(false);
    setUnderButtonR(false);
    setUnderButtonL(true, "Quit", "javascript:showPage('W_03.jsp')", "wiiCancelSound()");
    showElement("sdbuttons");
...
function showDetails()
{
    ...
    showElement("details");
    setUnderButtonR(true, "More Details<BR>(WIP)", "javascript:showPage('B_06.jsp?titleId=<%= id %>')", "wiiSelectSound()");
    <% if (request.getParameter("cardType") != null && request.getParameter("cardType").equals("gameCard")) { %>
    setUnderButtonL(true, "Back", "javascript:showBackCode()", "wiiCancelSound()");
    <% } else { %>
...
function onGiftRecvPressed(titleId) {
  setUnderButtonL(false);
  setUnderButtonR(false);
  hideElement("purchasePage");
  showElement("giftText");
  document.getElementById("accOrRej").innerHTML = "<b>Now receiving your gift...</b>";
  shop.beginWaiting();
  opName = "Accept gift title";
  opDesc = "Accepting gifted title";
  //we will use reason as the recipient friend code
  progress = ec.acceptGiftTitle(titleId, "", true, nwc24.myUserId);
  finishOp(opName, opDesc, progress, "onGiftRecvDone");

}

function onGiftRejPressed(titleId) {
  setUnderButtonL(false);
  setUnderButtonR(false);
  hideElement("purchasePage");
  showElement("giftText");
  document.getElementById("accOrRej").innerHTML = "<b>Now returning your gift...</b>";
  document.getElementById("heading").innerHTML = "Returning your gift";
  shop.beginWaiting();
  opName = "Reject gift title";
  opDesc = "Rejecting gifted title";
  progress = ec.acceptGiftTitle(titleId, "", false, nwc24.myUserId);
  finishOp(opName, opDesc, progress, "onGiftRejDone");
}

function onGiftRecvDone(progress) {
  showResult(progress, opName, opDesc);
  //Progress status will always be -4006 since it expects an ETicket 
  //to be present in the SOAP response (i believe)
  showSDSelectionAfterGift();
}

function onGiftRejDone(progress) {
  showResult(progress, opName, opDesc);
  document.getElementById("accOrRej").innerHTML = "<b>You returned your gift.</b>";
  setUnderButton(true, "OK", "javascript:showPage('W_03.jsp');", "wiiSelectSound()");
}
...
function onBuyButtonPressed(titleId, itemId, sd)
{
  ...
  nextUrl = addParam(nextUrl, 'gift', true);  
  <% } %>
	if (itemId == null) {
...
if (request.getParameter("from") != null) { %>
    ...
    if (purchaseButton == "NULL"){
        ...
        showElement("buyButton");
        if (purchaseButton == "UPDATE"){//update
			setFree('Update');
        }else if(purchaseButton == "RE"){//redownload
			document.getElementById("text03-01").innerHTML='Download';
        }else if(purchaseButton == "ICR"){//free title
			setFree('One Free Title');
            document.getElementById("BuyAnchor").href ='javascript:showICRWarningPage()';
        }else if(purchaseButton == "SCAGOLD"){//css
			setFree('Connection Ambassador');
        }else if(purchaseButton == "SCASILVER"){//sca
			setFree('Connection Ambassador');
        }else if(purchaseButton == "FREE"){//free
			setFree('Free');
        }else if (purchaseButton == "PRICE"){//price is more than 0
          setFree("Buy");
        }else{//PRICE
            trace("Price!!!");
            document.getElementById("text03-01").innerHTML = 'Download';
  ...
  if(shop.isCompatibleMode){
    ...
    //if(purchaseButton != "NULL" && giftButton != "NULL"){
        document.getElementById("buyButton").style.left = '52px';
    if(purchaseButton != "NULL" && giftButton == "NULL"){
        document.getElementById("buyButton").style.left = '52px';
...
function initPage()
{
    ...
    MM_preloadImages('/oss/oss/common/images//banner/under_banner_b.gif',
            ...
            '/oss/oss/common/images//banner/controller_msg2_<%= imgregion %>_<%= language %>.gif',
    ...
    <% if (titleId.equals("00010000534F5550") || titleId.equals("00010000534F5545") || titleId.equals("00010000534F554B") || titleId.equals("00010000534F554A") || titleId.equals("000100014F484243")) { %>
    hideElement("buyButton");
    showElement("notAvailableMsg");
    showBuyButtons(false);
    hideElement("buyButton");
    <% } else if (titleId.equals("0000000000000000")) { %>
    // function showUndeliverable
    hideElement("details");
    showBuyButtons(false);
    setUnderButtonR(false);
    ...
    hideElement("line01");
    showElement("upperLineLong")
    setUnderButtonR(false);
    setUnderButtonL(true, "Back", "javascript:showTransactions()", "wiiCancelSound()");
    <% } else { %>
    setUnderButtonL(true, "Back", "javascript:showBack()", "wiiCancelSound()");
    setUnderButtonR(true, "More Details<BR>(WIP)", "javascript:showPage('B_06.jsp?titleId=<%= id %>')", "wiiSelectSound()");
    <% } %>
    ...
    if (titleId == "00010000534F5550" || titleId == "00010000534F5545" || titleId == "00010000534F554A" || titleId == "00010000534F554B" || titleId == "000100014F484243") {
      hideElement("buyButton");
      showElement("notAvailableMsg");
      showBuyButtons(false);
      return; // PERMANETNT FIX FINALLY??? OMFGGG TS PMOOO
    }

    if (titleId == "0000000000000000") {
      hideelElement("details");
      showBuyButtons(false);
      showElement("noTitleMsg");
      setUnderButtonR(false);
      return;
...
if (request.getParameter("from") != null) { %>
  <script>
    setUnderButtonR(false);
    var nwc24 = new wiiNwc24();
    var friendSender = '<%= request.getParameter("from") %>';
    var friendName;
    for (var i = 0; i < nwc24.getFriendNum(); i++) {
      var friend = nwc24.getFriendInfo(i, "userId");
      if (friend == friendSender) {
        friendName = nwc24.getFriendInfo(i, "name");
      }
    }
    document.write('<div align="left" class="titleBlackL"><span id="heading">A Gift from ' + friendName + '</span></div>')
  </script>
<% } else {
...
<div id="details">
  ...
  <div style="overflow: hidden;" class="catalogTitleBlack_01" id="DisplayCategory" align="left">
    <%= genre %></div>
  <% if (!oldDate.equals("")){ %>
  <%-- remove "Released" for titles that dont have a release date  --%>
  <div class="contentsBlackS" id="date">
    <div align="left" class="catalogTitleBlack_01"><%= releaseDate %></div>
  </div>
  <% } %>
    <div id="Rating">
  ...
  <div class="catalogTitleBlack_01" id="Publisher">
    <div align="left"><% if (!publisher.equals("")) { %><%= publisher %><% } else { %>???publisher.???<% } %></div>
  </div>
  <div id="Players">
  ...
  <div id="Platform">
    <div align="left" class="headerWhiteS"><% if (!platform.equals("")) { %><%= platform %><% } else { %>???platform.???<% } %></div>
  </div>
  <div id="controller">
    <img src="/oss/oss/common/images/banner/controller_msg1_<%= imgregion %>_<%= language %>.gif" width="77" height="65" id="Image3control" />
  </div>
  <div id="controllerLink">
      ...
      <img src="/oss/oss/common/images//spacer.gif" width="77" height="65" border="0" id="Image2control" 
       onmouseover="MM_swapImage('Image3control','','/oss/oss/common/images//banner/controller_msg2_<%= imgregion %>_<%= language %>.gif',1);wiiFocusSound();" 
       onmouseout="MM_swapImgRestore()" onclick="wiiSelectSound();"/>
      </a>
    </div>

  <% if (WIFITYPE.equals("WFC")) { %> <%-- for titles that use NWFC/WFCPay&Play --%>
  <div id="Genre_WIFIFREE_B_05_E">
      <img src="/oss/oss/ext/attributes/images/genre/Genre_WIFIFREE_B_05_<%= imgregion %>.png" name="genre" border="0" id="genre">
    </div>
  <% } else if (WIFITYPE.equals("WFCPAY")) { %>
  <div id="Genre_WIFIPAY_B_05_E">
      <img src="/oss/oss/ext/attributes/images/genre/Genre_WIFIPAY_B_05_<%= imgregion %>.png" name="genre" border="0" id="genre">
    </div>
  <% } else if (WIFITYPE.equals("WIIMMFI")) { %>
    <div id="Genre_WIFIPAY_B_05_E">
      <img src="/oss/oss/ext/attributes/images/genre/Genre_WIIMMFI_B_05_ALL.png" name="genre" border="0" id="genre">
    </div>
...
<div id="buyButton" style="left: 52px">
  ...
  <% } else { %>
  <div id="buyButton" style="left: 52px;top: 293px;">
    <div id="buybanner"><img src="/oss/oss/common/images/banner/getgift_a.png" width="241" height="76" id="Image3" /></div>
    <div id="giftrecvtxt" class="buttonTextWhiteL buttonText">
        Receive</div>
    <div id="buyspacer">
      <a id="BuyAnchor" href="javascript:onGiftRecvPressed('<%= titleId %>')">
        <img src="/oss/oss/common/images//spacer.gif" width="241" height="76" border="0" id="Image2" 
         onmouseover="MM_swapImage('Image3','','/oss/oss/common/images//banner/getgift_b.png',1);wiiFocusSound();" 
         onmouseout="MM_swapImgRestore()" 
         onclick="wiiSelectSound();"/>
      </a>
    </div>
  ...
  <div id="giftRej" style="position:absolute; left:321px; top:293px; width:241px; height:76px;">
    <div id="giftBanner" style="position:absolute; left:0px; top:0px; width:241px; height:76px; z-index:32; ">
      <img src="/oss/oss/common/images//banner/rejectgift_a.png" width="241" height="76" id="giftImage3"></div>
    <div id="giftrejtxt" class="buttonTextWhiteL buttonText">
      Return</div>
    <div id="giftSpacer" style="position:absolute; left:0px; top:0px; width:100%; height:100%; z-index:35; ">
      <a id="giftAnchor" href="javascript:onGiftRejPressed('<%= titleId %>')">
...
<div id="sdbuttons" style="display:none">
  ...
  <% if (request.getParameter("B_05Debug") != null) { %>
...
<!--Gift receive/reject text-->
<div class="catalogTitleBlue" id="giftText" style="display:none;">
  <div style="overflow:hidden" nowrap class="contentsBlue" id="GiftName1">
    <div align="center" class="headerBlueM"><%= title1 %></div>
  </div>
  <div style="overflow:hidden" nowrap class="contentsBlue" id="GiftName2">
      <div align="center" class="headerBlueM"><%= title2 %></div>
    </div>
    <div class="titleBlueM" id="accOrRej" align="center"></div>
</div>
