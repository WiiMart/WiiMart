<%@ page import = "org.wiimart.Catalog,org.wiimart.Title,java.text.SimpleDateFormat,java.util.stream.Collectors,java.math.*,java.sql.*,java.io.*,java.util.*,java.net.http.*,java.net.URI,java.net.http.HttpResponse.BodyHandlers,java.net.HttpURLConnection,java.net.URL,java.nio.charset.StandardCharsets,org.json.*,javafx.scene.web.*" %>
<%@ page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %><%@ page buffer="8192kb" autoFlush="true" %>
<%@ taglib uri = "http://java.sun.com/jsp/jstl/core" prefix = "c" %>
<% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%><a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a><a href="javascript:window.location.reload();"> reload</a><% } boolean errored = false; int pageParam = request.getParameter("p") != null ? Integer.parseInt(request.getParameter("p")) : 1; %><%
class GiftTransaction {
  ...
  public GiftTransaction(String id, String fromWho, String toWhom, String ts, boolean acc, boolean rej) {
    this.titleId = id;
    this.from = fromWho;
    this.to = toWhom;
    this.timestamp = ts;
...
<!--<%
  ...
  //check if user exists in db with the given device credentials
  Connection conn = DriverManager.getConnection(url, props);
  String updateSQL = "SELECT * FROM public.userbase WHERE account_id = ? AND device_id = ? AND device_token_hashed = ? AND serial_number = ?";
  PreparedStatement pst = conn.prepareStatement(updateSQL);
  pst.setInt(1, Integer.parseInt(accountId));
  pst.setLong(2, Long.parseLong(deviceId));
  ...
  if (!rs.next()) {
    ...
    realPoints = Integer.parseInt(rs.getString("points"));
  } %>
function isRefreshBalanceNeeded()
{
    var realBalance = parseInt('<%= realPoints %>');
...
function alignHeader(contentHeader, headerWrapper){
    ...
    if (h1) {
        ...
        trace("h1 has offsetHeight " + headerH);
        var h2 = document.getElementById(headerWrapper);
        if (h2) {
            if (headerH > 45) {
                trace("setting " + headerWrapper + " top to 84px");
                h2.style.top='84px';
            } else if (headerH > 25) {
                trace("setting " + headerWrapper + " top to 93px");
                h2.style.top='93px';
            } else {
                trace("setting " + headerWrapper + " top to 115px");
                h2.style.top='115px';
            }
        }
    }
...
{
	initPageCommon();<%
  ArrayList<GiftTransaction> gifts = null;
  if (accountId == null || deviceId == null || serialNo == null || token == null || deviceCode == null) { %>
  MM_preloadImages('/oss/oss/common/images//banner/aa_banner_b.gif');
  var next = '';
	if (next == null || next == '') {
	...
	setErrorMsg('206601', errorMsg);<% errored = true;} else { 
  

  //check pending gifts
  gifts = new ArrayList<GiftTransaction>();
  if (!errored) {
    updateSQL = "SELECT titleid, fromwho, towhom, accepted, rejected, timestamp FROM gifted_titles WHERE towhom = ?";
    pst = conn.prepareStatement(updateSQL);
    pst.setString(1, deviceCode);
    rs = pst.executeQuery();
    while (rs.next()) {
      gifts.add(new GiftTransaction(rs.getString("titleid"), rs.getString("fromwho"), rs.getString("towhom"), rs.getString("timestamp"), rs.getBoolean("accepted"), rs.getBoolean("rejected")));
    }
    for (GiftTransaction gift : gifts) {
      long millis = Long.parseLong(gift.timestamp);
      java.util.Date date = new java.util.Date(millis);
      gift.timestamp = new SimpleDateFormat("MM/dd/yy").format(date);
    }%>
    shop.endWaiting();
    if (ecSupportsSession()) {
    ...
    if (refreshNeeded) {
<c:choose>
  <c:when test="${param.og eq true}">
setF01Title('<div align="left" class="titleBlackL">Account Activity</div>');
  </c:when>
  <c:otherwise>
setF01Title('<div align="left" class="titleBlackL">Pending Gifts</div>');
  </c:otherwise>
</c:choose>
    ...
    trace("text of contentM01: " + document.getElementById("contentM01").innerHTML);
    trace("text of contentM02: " + document.getElementById("contentM02").innerHTML);
    trace("text of contentM03: " + document.getElementById("contentM03").innerHTML);
    trace("text of contentM04: " + document.getElementById("contentM04").innerHTML);
...
<% if (errored) { %>
	  <div class="dot" id="upperLineLong">･･･････････････････････････････････････････････････････････････････････････</div>
...
<div align="left">Account Activity</div>
  ...
  <c:otherwise>
<div align="left">Pending Gifts</div>
  </c:otherwise>
</c:choose>
</div>
<div id="pageTotal" class="buttonTextBlackM"><%= pageParam %>/<%= (int) Math.ceil(gifts.size() / 4) + 1 %></div>
<% if (pageParam != 1) { %>
<div id="arrowL" style="position:absolute; width:52px; height:52px; z-index:38; left: 398px; top: 380px;">
    <a href="javascript:showTransactions('<%= pageParam - 1 %>')" 
     onmouseout="MM_swapImgRestore()" 
...
<% } %>
<% if (pageParam != (int) Math.ceil(gifts.size() / 4) + 1) { %>
...
<div id="header01">
    ...
    <div id="header04">
      <span id="pointHeader" style="line-height:16px" class="headerBlueS">Accepted</span>
    </div>
    <div id="header05">
      <span id="balanceHeader" style="line-height:16px" class="headerBlueS">Rejected</span>
    </div><%--
    public static JSONArray slice(JSONArray originalArray, int startIndex, int endIndex) throws JSONException {
        JSONArray slicedArray = new JSONArray();
        for (int i = startIndex; i < endIndex && i < originalArray.length(); i++) {
            slicedArray.put(originalArray.get(i));
        }
        return slicedArray;
    }
    ...
    int startNum = (pageParam - 1) * 4;
    int endNum = startNum + 4;
    if (endNum > gifts.size()) {
        endNum = gifts.size();
    }
    //slice gifts arraylist
    ArrayList<GiftTransaction> newGifts = new ArrayList<GiftTransaction>();
    //Start from the end because the most recent gifts are at the end
    //tbh idk why this was a normal for-i loop before
    //no clue what it was used for but whatever
    for (int i = endNum-1; i > startNum-1; i--) {
        newGifts.add(gifts.get(i));
    }
    for (int i = 1; i < newGifts.size() + 1; i++) { 
    GiftTransaction gift = newGifts.get(i - 1); %>
    <div id="row0<%= i %>">
        <div class="contentsBlack" id="date">
          <div align="center">
            <%= gift.timestamp %></div> <!-- first date -->
        </div>

        <div class="contentsBlack" id="type0<%= i %>" style="position:absolute;left:89px;top:12px;width:136px;height:18px;z-index:82;">
            <div id="typeM0<%= i %>" align="center">
            A gift from 
            <br> 
            <script>
                ...
                for (var i = 0; i < nwc24.getFriendNum(); i++) {
                    ...
                    if (friend == '<%= gift.from %>') {
                        document.write(nwc24.getFriendInfo(i, "name"));
                    }
                }
            </script></div> 
        </div>
        ...
        <div class="contentsBlack" id="content">
          ...
          <%
            ...
            options.put("titleId", gift.titleId);
          %>
            <%= catalog.getTitle(options).title1 %></div> <!-- first tn -->
        </div>

    
      <% if (!gift.accepted && !gift.rejected) { %>
      <div id="aabanner">
        <img src="/oss/oss/common/images/banner/aa_banner_a.gif" width="151" height="42" id="Image1<%= i %>" name="Image1<%= i %>"/>
      </div>
      <div id="aabannerspace">
        <a href="javascript:showPage('B_05.jsp?titleId=<%= gift.titleId %>&from=<%= gift.from %>')" onmouseover="snd.playSE( cSE_Forcus );" onclick="snd.playSE(cSE_Decide);">
          <img src="/oss/oss/common/images//spacer.gif" width="151" height="42" border="0" id="Image<%= i %>" 
           onmouseover="MM_swapImage('Image1<%= i %>','','/oss/oss/common/images//banner/aa_banner_b.gif',1)" 
           onmouseout="MM_swapImgRestore()" />
        </a>
      </div>
      <% } %>
    <div id="cost">
          <div align="right" class="contentsBlack">
            <div align="center"><%= gift.accepted == true ? "Yes" : "No" %></div></div> <!-- first removedBalance -->
        </div>
        <div id="balance">
          <div align="right" class="contentsBlack">
            <div align="center"><%= gift.rejected == true ? "Yes" : "No" %></div> <!-- first balanceAfterPurchase -->
          </div>
        </div>
    <img src="/oss/oss/common/images/banner/account_activity.png" width="555" height="42" />
      </div>
      <% if (!gift.accepted && !gift.rejected) { %>
      <div id="aabannershadow" style="position:absolute; left:245px; top:<%= 132 + (59 * (i - 1)) %>px; width:170px; height:58px; z-index:78">
...
<div align="left"><script>document.write((typeof(shop) == "undefined" ? "Wii Shop Channel" : shop.title))</script></div>
</div>
<% } %>
