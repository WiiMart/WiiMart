...
<%
    String method = request.getParameter("method") == null ? "Enable" : request.getParameter("method");
    String serial = request.getParameter("serialNo") == null ? "" : request.getParameter("serialNo");
    boolean ogMode = method.equals("Enable") ? true : false;
    ...
    String updateSQL = "UPDATE public.userbase SET og_mode = ? WHERE serial_number = ?";
    PreparedStatement pst = conn.prepareStatement(updateSQL);
    pst.setBoolean(1, ogMode);
...
function initPage()
{
	...
	if ('<%= ogMode %>' == "false") {
		...
		ec.setSessionValue("currTitle", "Wii Shop Channel")
	}
...
<div id="text01-01" class="titleBlackL">OG Mode</div>
<div class="titleBlackL" id="text02-01" style="text-align: center;">
    <%
        if (ogMode) {%>
            You enabled OG mode!
        <% } else { %>
            You disabled OG mode :(
        <% } %>
    </div>
</body>
