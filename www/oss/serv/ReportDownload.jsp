<%@ page import = "java.io.*,java.util.*,java.sql.*" %><%@ page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %><% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%><a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a><% } %><%
String serialNo = request.getParameter("serialNo") == null ? "" : request.getParameter("serialNo");
int pointsToDeduct = request.getParameter("pt") == null ? 0 : Integer.parseInt(request.getParameter("pt"));
...
String updateSQL = "SELECT points FROM public.userbase WHERE serial_number = ?";
...
int newPoints = points - pointsToDeduct;
updateSQL = "UPDATE public.userbase SET points = ? WHERE serial_number = ?";
PreparedStatement pst1 = conn.prepareStatement(updateSQL);
pst1.setInt(1, newPoints);
pst1.setString(2, serialNo);
...
function initPage()
{
	...
	showPage('B_10.jsp?titleId=<%= request.getParameter("titleId") == null ? "" : request.getParameter("titleId") %>&reportDownloadDone=true&SD=<%= request.getParameter("SD") == null ? "N" : request.getParameter("SD") %>');
}
//-->
</script>

<body onload="initPage();var shop = new wiiShop();var unused = shop.connecting;"></body>
</html>
