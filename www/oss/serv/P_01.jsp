...
<% if (request.getParameter("region") != null && request.getParameter("language") != null && request.getParameter("region").equals("JPN") && request.getParameter("language").equals("jp")) { %>
<SCRIPT language="JavaScript">
	if (shop.isCompatibleMode) {
		showPage('P_01_JP_WiiU1st.jsp?region=<%= request.getParameter("region") %>&country=<%= request.getParameter("country") %>&region=<%= request.getParameter("region") %>', true);
	} else {
		showPage('P_01_JP.jsp?region=<%= request.getParameter("region") %>&country=<%= request.getParameter("country") %>&region=<%= request.getParameter("region") %>', true);
	}
</SCRIPT>
<% } %>
...
{
    if(snd) snd.playSE(cSE_Decide);
	trace("isCompatibleMode: " + shop.isCompatibleMode);
	if (shop.isCompatibleMode) {
...
<span id="P_01_WiiU">Add Wii Points to your <% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %>Wii Shop Channel<% } %> account?</span>
      ...
      <div id="word" style="display:'';top:7px">
