...
function initPage()
{
    ...
    if (isShowUpdateAvailable()) {
        if ( shop.isCompatibleMode )
        {
            document.getElementById("updateMessage").innerHTML = "There is an update to <% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %> the Wii Shop Channel<% } %>!<BR><BR>To receive this update, go to System Settings on the Wii U Menu and select System Update. Come back after you&rsquo;ve updated!"
...
<div class="titleBlueL" id="text01-01">
  <span id="wiiTitle"><script type="text/javascript">document.write(shop.title);</script></span>
</div>
<div align="center" class="buttonTextBlackM" id="text02-01">
   <span id="updateMessage">There is an update to <% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %>the Wii Shop Channel<% } %>!<BR><BR>To receive this update, go to Wii Settings and select Wii System Update. When you return to  <% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %> the Wii Shop Channel<% } %>, you will receive the update. Come back after you&rsquo;ve updated!</span>
</div>
