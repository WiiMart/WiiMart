...
function initPage()
{
	...
	setUnderButtonR(true, "Download", "javascript:showPage('B_05.jsp?titleId=<%= titleId %>&fromb06true=true')", "javascript:wiiSelectSound()");
	<% } %>
...
<div id="text03-01" class="catalogTitleBlack_01">
	<% if (titleId == null || titleId.equals("")) { %>
	<% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %>The Wii Shop Channel<%}%> isn&rsquo;t just about downloading Virtual Console games 窶錀 we&rsquo;ve also created new software just for Wii. One example is the Internet Channel.<BR><BR>The Internet Channel allows you to surf the web from the comfort of your couch. We&rsquo;re planning to release the Internet Channel in the near future. Check back often for this and other exciting Channels.
	<% } else { %>
		<% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%>WiiMart<% } else { %>The Wii Shop Channel<%}%> isn’t just about downloading Virtual Console games -- we’ve also created new software just for Wii. One example is the Internet Channel.<BR><BR>The Internet Channel allows you to surf the web from the comfort of your couch. We’re planning to release the Internet Channel in the near future. Check back often for this and other exciting channels.
	<% } %>
	</div>
<div id="Layer1">
