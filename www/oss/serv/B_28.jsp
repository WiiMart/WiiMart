...
<%@ page import = "java.io.*,java.util.*,java.net.http.*,java.net.URI,java.net.http.HttpResponse.BodyHandlers,java.net.HttpURLConnection,java.net.URL,java.nio.charset.StandardCharsets,org.json.*,javax.servlet.*,org.json.*,java.nio.file.*,java.io.*" %>
...
String imgregion = "E"; // FOR WIFIPARTIALLYCHARGED/WIFIALLCHARGED/WIFI INDICATOR
switch (region) {
    case "EUR":
      imgregion = "P";
      break;
	...
	Path path = Paths.get(application.getRealPath("/WEB-INF/classes/SelectionConfirmation.json"));
    ...
    for (int i = 0; i < TitleDetailsArray.length(); i++) {
        ...
        if (titleId.equals(JSONTitleId)) {
			request.setAttribute("Body", detailsarray.getString("Body"));
			request.setAttribute("WIFIPARTIALLYCHARGED", detailsarray.getString("WIFIPARTIALLYCHARGED"));
			request.setAttribute("WIFIALLCHARGED", detailsarray.getString("WIFIALLCHARGED"));
        }
    }
...
function initPage()
{
	...
	if (ecSupportsSession()) {
		...
		var hasCautionItem3 = '<% if (titleId.equals("000100015746534A")) { %>true<% } else { %>false<% } %>';
		...
		if (giftStatus == "sending") {
			...
			if (hasCautionItem3 == "true") {
				setUnderButtonR(true, "OK", "javascript:showPage('B_29.jsp?titleId=<%= titleId %>&itemId=&campaignCode=&SD="+sd+"')", "snd.playSE(cSE_Decide)");
			} else {
				setUnderButtonR(true, "OK", "javascript:showPage('B_18.jsp?titleId=<%= titleId %>&itemId=&SD="+sd+"')", "snd.playSE(cSE_Decide)");
			}
			...
			if (hasCautionItem3 == "true") {
				...
				setUnderButtonR(true, "OK", "javascript:showPage('B_09.jsp?titleId=<%= titleId %>&SD="+sd+"')", "snd.playSE(cSE_Decide)");
			}
		...
		/* } else if (redownloadFlag == "true") {
			setUnderButtonL(true, "Back", "javascript:showGiftTrans("+transId+")", "snd.playSE(cSE_Cancel)");
			setUnderButtonR(true, "OK", "javascript:showPage('B_09.jsp?titleId=0&SD="+sd+"')", "snd.playSE(cSE_Decide)");
		*/
		} else {
			setUnderButtonL(true, "Back", "javascript:showBack()", "snd.playSE(cSE_Cancel)");
...
<% if (!title2.equals("")) { %>
<div id="TitleName2stline" nowrap>
...
<div id="cautionArea">
	<c:if test="${WIFIPARTIALLYCHARGED eq 'true'}">
  <div align="center" style="height:300px;">
    <div id="wifiPartiallyCharged" style="padding:7px;" align="center">
      <img src="/oss/oss/common/images/banner/B_28_WIFIPAY_<%= imgregion %>.gif" width="429" height="75" />
	...
	<c:if test="${WIFIALLCHARGED eq 'true'}">
    <div id="wifiAllCharged" style="padding:7px;" align="center">
      <img src="/oss/oss/common/images/banner/B_28_WIFIPAY_<%= imgregion %>.gif" width="429" height="75" />
  ...
  <table align="center" cellpadding="4"><tr><td align="left">
	<span class="contentsBlackS">
