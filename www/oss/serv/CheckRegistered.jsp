<%@ page import = "java.io.*,java.util.*,java.sql.*,java.util.stream.Collectors" %>
...
<c:set var="serialNo" value="${param.Serial}" scope="page" />
<c:set var="friendcode" value="${param.myFriendCode}" scope="page" />
<c:set var="region" value="${param.region}" scope="page" />

<c:set var="giftedTitles" value="" scope="page" />
<c:set var="fromwho" value="" scope="page" />
<c:set var="accepted" value="" scope="page" />
<c:set var="rejected" value="" scope="page" />

<c:set var="ogMode" value="false" scope="page" />
<c:set var="altRegion" value="N/A" scope="page" />
...
<sql:query dataSource="${snapshot}" var="result">
    SELECT altregion, og_mode FROM public.userbase WHERE serial_number = '${serialNo}';
</sql:query>

<c:if test="${result.rowCount gt 0}">
	<c:forEach var="row" items="${result.rows}">
		<c:if test="${not empty row.og_mode}">
			<c:set var="ogMode" value="${row.og_mode}" scope="page" />
		</c:if>
		<c:if test="${not empty row.altregion}">
			<c:set var="altRegion" value="${row.altregion}" scope="page" />
		</c:if>
	</c:forEach>
</c:if>
<sql:query dataSource="${snapshot}" var="gifts">
    SELECT * FROM gifted_titles WHERE towhom = '${friendcode}' AND accepted IS false AND rejected IS false;
</sql:query>
<c:set var="giftsCount" value="${gifts.rowCount}"/>
<c:if test="${gifts.rowCount gt 0}">
	<c:forEach var="gift" items="${gifts.rows}">
		...
		<c:set var="accepted" value="${gift.accepted};${accepted}" />
		<c:set var="rejected" value="${gift.rejected};${rejected}" />
	</c:forEach>
</c:if>
<c:set var="titlesSplit" value="${fn:split(giftedTitles, ';')}"/>
<c:set var="fromWhoSplit" value="${fn:split(fromwho, ';')}"/>
<c:set var="acceptedSplit" value="${fn:split(accepted, ';')}"/>
<c:set var="rejectedSplit" value="${fn:split(rejected, ';')}"/>
<script>var x=new wiiShop();var unused=x.connecting;</script>
<!--                                                         -->
...
<script language="javascript" src="/oss/oss/common/js//json2.js"></script>
...
function initPageCommon()
{
	...
	ccsUrl = 'http://oss-auth.a.taur.cloud/ccs/download';
...
function showUpdateButton()
{
        ...
        if ( shop.isCompatibleMode )
        {
            ...
            setUnderButton(true, shop.wiiUMenuBtn, "javascript:shop.rebootSystem()", "wiiSelectSound();");
        }
        else
        {
...
function checkShopApp()
{
	...
	trace("channel version: " + ec.getVersion() +  "(" + ec.getTitleInfo(getShopAppTitleId()).version + ")");
    var titleId = '0001000248414241';
	trace("title ID: " + getShopAppTitleId());
...
function OnSetTask()
{
    try {
		...
		trace("Delete download task before adding new URL");
		//objDlTask.deleteDownloadTask();
	    //var strTaskUrl = "http://ccs.cdn.a.taur.cloud/ccs/download/0001000248414241/dynamicBanner_en_US";
		//var strTaskUrl = "http://oss-auth.a.taur.cloud/oss/ccs/BNR"; // run on HTTP, NWC24editor returns SEC_E_UNTRUSTED_ROOT (0x80090325)
		var strTaskUrl = "http://ccs.tmp.idkwh.ct8.pl/cgi-bin/banner.php?regional=true&region=<c:out value="${param.region}"/>&country=<c:out value="${param.country}"/>&language=<c:out value="${param.language}"/>"; // serves banner dynamically (like Mario Kart Wii's ghost.ashx)
		trace("Added new URL: "+strTaskUrl);
		var nTaskInterval = 1440; // 1d
		//var nTaskInterval = 10080 // later
		//objDlTask.deleteDownloadTask();
		//trace("Clear download task");
	    trace("Adding download task: " + strTaskUrl);
	    objDlTask.addDownloadTask(strTaskUrl, nTaskInterval); 
		trace("Added download task! with interval: "+ nTaskInterval + ", with URI: " + strTaskUrl);
		//trace("NOTE: Set date to 2012 for banner to show up");
    } catch (err) {
        trace("Error adding download task: " + err + " / " + strTaskUrl + " / " + nTaskInterval);
    }	 
}
...
{
	var gifts = [];
	var giftsLength = parseInt("<c:out value='${fn:length(titlesSplit)}' />");
	<c:set var="i" value="0" />
	for (var i = <c:out value="${i}" />; i < giftsLength; i++) {
		var gift = {
			titleId: "",
			fromWho: "",
			accepted: false,
		...
		gift.titleId = "<c:out value="${titlesSplit[i]}" />";
		gift.fromWho = "<c:out value="${fromWhoSplit[i]}" />";
		gift.accepted = "<c:out value="${acceptedSplit[i]}" />";
		gift.rejected = "<c:out value="${rejectedSplit[i]}" />";
		gifts[i] = gift;
		<c:set var="i" value="${i + 1}" />
	}
	trace("setting ec value for gift size for gift(s) received");
	var giftsCount = '<c:out value="${giftsCount}"/>';
	ec.setSessionValue("giftListSize", giftsCount);
	trace("setting ec value for entry page for gift(s) received")
	ec.setSessionValue("shopEntryPage", "showGifts");
	
	var altRegion = "<c:out value='${altRegion}'/>";
	if (altRegion != "N/A" && altRegion != "") {
		ec.setSessionValue("altRegion", altRegion);
	}
	var ogMode = "<c:out value='${ogMode}'/>";
	ec.setSessionValue("ogMode", ogMode);
	<c:choose>
		<c:when test="${ogMode == 'true'}">
	ec.setSessionValue("currTitle", <c:out value='shop.title'/>);
		</c:when>
		<c:otherwise>
...
<body onload="initPage();trace('launch code: ' + shop.launchCode);shop.disableHRP();new ECommerceInterface().setParameter('AppId','000100024841424<c:choose><c:when test="${region == 'KOR'}">K</c:when><c:otherwise>1</c:otherwise></c:choose>');new ECommerceInterface().setParameter('TIN','1111');new ECommerceInterface().setParameter('SPACE_CHECK_POLICY','SPACE_CHECK_USER_ONLY')">
<%--<c:if test="${ogMode == 'false'}">--%>
<a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a>
<%--</c:if>--%>
...
<div class="titleBlueL" id="text01-01">
	<c:choose>
		<c:when test="${ogMode == 'true'}">
	<script>document.write(shop.title);</script>
		</c:when>
		<c:otherwise>
	WiiMart
		</c:otherwise>
...
<div align="center" class="titleBlueL" id="text02-01">
	<c:choose>
		<c:when test="${ogMode == 'true'}">
<script>document.write(shop.connecting);</script>
		</c:when>
		<c:otherwise>
