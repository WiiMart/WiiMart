...
<%
    String[] strLang = {"System Language", "English (US)", "Italian", "German", "Spanish (US)", "French", "Korean", "Japanese"};
    String[] strLangVal = {"sys", "en", "it", "de", "mx", "fr", "ko", "ja"};
    String[] strLangProgress = {"?", "100", "25", "0", "20", "10", "50", "50", "20"};
%>
<!--  -----------------------------------------------------  -->
...
<% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%><a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a> <a href="javascript:location.reload()">reload</a><% } %>
...
function initPage()
{
	...
	'/oss/oss/common/images//banner/top_b.gif','/oss/oss/common/images//banner/B_17_publisher_a_01.png');
	setUnderButtonL(true, "Back", "javascript:showSettings()", "snd.playSE(cSE_Cancel)");
}
var scroll_step = 30;
function kdown()
{
...
function select(method) {
		...
		form.action = getSecureUrl("changeLanguage.jsp");
		form.id = "langChangeForm";
		form.innerHTML = "";
		form.innerHTML += '<div id="commonFields"></div>';
		form.innerHTML += '<input type="hidden" name="targetLanguage" value="' + method + '"/>';
...
<body onkeypress="kdown();" onload="initPage(); searchItem.focus();" style="overflow-x: hidden;">
<!--  -----------------------------------------------------  -->
<!--  Copyright 2005-2014 Acer Cloud Technology, Inc.        -->
<!--  All Rights Reserved.                                   -->
<!--                                                         -->
...
<div id="text01-01">
  <div align="left"><span class="titleBlackL">Select a Language</span></div>
</div>
...
<div id="publisherBanner0<%= i + 1 %>" style="position:absolute; left:<%= (i % 2 == 0) ? 6 : 241 %>px; top:<%= 2 + Math.floor(i / 2) * 54 %>px; width:235px; height:55px;">
      <div id="banner0<%= i + 1 %>" style="position:absolute; left:0px; top:0px; width:235px; height:53px; z-index:5;">
        <img src="/oss/oss/common/images//banner/B_17_publisher_a_00.png" id="Image0<%= i + 1 %>" />
      </div>
      <div id="word0<%= i + 1 %>" style="position:absolute; left:6px; top:6px; width:223px; height:22px; z-index:6; overflow: hidden;" align="center">
        <table height="100%">
        <tr>
          <td align="left" valign="middle">
            <span class="contentsBlack"><%= strLang[i] %></span>
          </td>
        </tr>
        </table>
      </div>
      <div id="hit0<%= i + 1 %>-01" style="position:absolute; left:116px; top:31px; width:84px; height:16px; z-index:6;">
        <table width="100%" height="100%">
        <tr>
          <td align="right" valign="top">
            <span class="contentsBlack">Progress: </span>
          </td>
        </tr>
        </table>
      </div>
      <div id="hit0<%= i + 1 %>-02" style="position:absolute;left:180px;left:196px; top:31px; width:31px; height:16px; z-index:6;">
        <table width="100%" height="100%">
        <tr>
          <td align="right" valign="top">
            <span class="contentsBlack"><%= strLangProgress[i] %>%</span>
          </td>
        </tr>
        </table>
      </div>
      <div id="spacer0<%= i + 1 %>" style="position:absolute; left:<%= (i % 2 == 0) ? 6 : 0 %>px; top:<%= (i % 2 == 0) ? 2 : 0 %>px; width:235px; height:55px; z-index:7;">
        <a href="javascript:select('<%= strLangVal[i] %>')">
          <img src="/oss/oss/common/images//spacer.gif" name="Image0<%= i+1 %>s" width="235" height="55" border="0" id="Image0<%= i+1 %>s" 
           onmouseover="MM_swapImage('Image0<%= i+1 %>','','/oss/oss/common/images//banner/B_17_publisher_a_01.png',1);snd.playSE( cSE_Forcus );" 
           onmouseout="MM_swapImgRestore()" 
           onclick="snd.playSE(cSE_Decide);" />
        </a>
      </div>
    </div>
<!--img src="/oss/oss/common/images//spacer.gif" width="1" height="1" id="Image0" /-->
<% } %>
</div>
</body>
</html>
