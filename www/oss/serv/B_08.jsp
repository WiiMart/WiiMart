...
<%@ page import = "java.net.http.*,java.net.URI,java.net.http.HttpResponse.BodyHandlers,java.net.HttpURLConnection,java.net.URL,java.nio.charset.StandardCharsets,org.json.*" %>
<%@ page import = "org.wiimart.Catalog,org.wiimart.Title,java.io.*,java.util.*,java.net.http.*,java.net.URI,java.net.http.HttpResponse.BodyHandlers,java.net.HttpURLConnection,java.net.URL,java.nio.charset.StandardCharsets,org.json.*,javax.servlet.*,org.json.*,java.nio.file.*,java.io.*" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page trimDirectiveWhitespaces="true" %>
<%@ page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %>
<% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%><a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a> <a href="javascript:window.location.reload();">reload</a><% } %>
<%--TESTING FOR ALL B_08 PAGES--%><%
...
String tID = request.getParameter("titleId") == null ? "_" : request.getParameter("titleId");
String sd = request.getParameter("SD") == null ? "N" : request.getParameter("SD");
String path = "";
if (tID.equals("_")) path = "/root/www/SelectionConfirmation/_.jsp";
else if (language.equals("jp")) path = "/root/www/SelectionConfirmation/" + tID + "_" + country + ".jsp";
else path = "/root/www/SelectionConfirmation/" + tID + "_" + country + "_" + language + ".jsp";
File file = new File(path);
String selectionConfirmation = "";
boolean foundB_08 = false;
if (file.exists()) {
	foundB_08 = true;
	out.println(new String(Files.readAllBytes(Paths.get(path)), "UTF-8").replaceAll("&SD=", "&SD=" + sd).replaceAll("B_28.jsp", "B_09.jsp"));
}
if (!foundB_08) { %>
<%--<% JSONObject jsonobj = new JSONObject(json);
    ...
    try {
		TitleDetailsArray = jsonobj.getJSONArray("Warning_" + region.toUpperCase() + "_" + language.toUpperCase());
	} catch (org.json.JSONException e) {
		TitleDetailsArray = jsonobj.getJSONArray("Warning_USA_EN");
	}

	String warnType = "B_09";
	String B_08_Warning = "";
    ...
    for (int i = 0; i < TitleDetailsArray.length(); i++) {
        ...
        if (titleId.equals(JSONTitleId)) {
			warnType = detailsarray.getString("WarningType");
			B_08_Warning = detailsarray.getString("B_08_Warning");
		}
    }
%>
<script>console.log("")</script>
<%
String targetURL = "http://127.0.0.1:8082/getTitle?titleId=" + titleId + "&region=" + region + "&language=" + language;
%>
<script>
    console.log("<%= titleId %>");
	trace("warntype: <%= warnType %>");
</script>
<%
...
function initPage()
{
	...
	if (ecSupportsSession()) {
		var sd = '<%= request.getParameter("SD") == null ? "N" : request.getParameter("SD") %>';
		trace("sdmode: " + sd);
		...
		if (giftStatus == "sending") {
			...
			if (showCtrlPage != "true") {
				if (hasCautionItem2 == "true" || ((genreWifiPay == "true") && (wifiIndicator == WIFI_PARTIALLY_CHARGED || wifiIndicator == WIFI_ALL_CHARGED || wifiIndicator == WIFI_FREE_INPUT))) {
					setUnderButtonR(true, "OK", "javascript:showPage('B_28.jsp?titleId=<%= tID %>&itemId=&SD=<%= sd %>')", "snd.playSE(cSE_Decide)");
				} else if (hasCautionItem3 == "true") {
					setUnderButtonR(true, "OK", "javascript:showPage('B_29.jsp?titleId=<%= tID %>&itemId=&SD=<%= sd %>')", "snd.playSE(cSE_Decide)");
				} else {
					setUnderButtonR(true, "OK", "javascript:showPage('<%= warnType %>.jsp?titleId=<%= tID %>&itemId=&SD=<%= sd %>')", "snd.playSE(cSE_Decide)"); // important part
				}
			}
			...
			if (showCtrlPage == "true") {
				...
				if (hasCautionItem2 == "true" || ((genreWifiPay == "true") && (wifiIndicator == WIFI_PARTIALLY_CHARGED || wifiIndicator == WIFI_ALL_CHARGED || wifiIndicator == WIFI_FREE_INPUT))) {
					...
					setUnderButtonR(true, "OK", "javascript:showPage('<%= warnType %>.jsp?titleId=<%= tID %>&SD=<%= sd %>')", "snd.playSE(cSE_Decide)"); // important part
				}
			}
		...
		/* } else if (redownloadFlag == "true") {
			setUnderButtonL(true, "Back", "javascript:showGiftTrans("+transId+")", "snd.playSE(cSE_Cancel)");
			setUnderButtonR(true, "OK", "javascript:showPage('B_09.jsp?titleId=<%= tID %>&SD=" + sd + "')", "snd.playSE(cSE_Decide)");
		*/
		} else {
...
<div id="text03-01">
  ...
  <div id="text04-01">
    <div align="center" >
      <div align="left" class="contentsBlackS">
		<%--<%= B_08_Warning %>--%>
	  </div>
    </div>
  </div>
