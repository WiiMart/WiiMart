...
function showCheckRegistered()
{
    ...
    var url = "https://oss-auth.a.taur.cloud/oss/serv/CheckRegistered.jsp";
	...
	%>
<%  if (initPage != null) {
	if (request.getParameter("initpage").equals("showPoints")) { %>
		var lc = shop.launchCode;
		ec.setSessionValue("initialPage", "javascript:showPoints()");
		ec.setSessionValue("shopEntryPage", "showPoints");
		url = addParam(url, "launchcode", lc);
	<% } else if (initPage.equals("showManual") && titleId != null) { %>
        var lc = shop.launchCode;
		ec.setSessionValue("initialPage", "javascript:showNotImplementedYet()");
		ec.setSessionValue("shopEntryPage", "showManual");
		url = addParam(url, "launchcode", lc);
	<% } else if (request.getParameter("initpage").equals("skipW_04")) { %> // useless as of now
		var lc = shop.launchCode;
		ec.setSessionValue("initialPage", "javascript:skipW_04();");
		ec.setSessionValue("shopEntryPage", "skipW_04");
		url = addParam(url, "launchcode", lc);
	<% } else if (initPage.equals("showHelp")) { %>
		ec.setSessionValue("initialPage", "javascript:showShoppingManual()");
		ec.setSessionValue("shopEntryPage", "showHelp");
		url = addParam(url, "launchcode", shop.launchCode);
	<% } else if (request.getParameter("titleId") != null) { 
		// showTitle : shows details (B_05)
		// showManual: shows a title manual
		// showGiftReceived: shows available gifts
		// showPoints: shows points page
		// skipW_04: CUSTOM, skip W_04, broken atm
		// showHelp: Shows the shopping manual 
	if (request.getParameter("titleId").equals("DEBUG") && initPage.equals("showTitle")) { %>
		trace("doing DEBUG");
		url = "https://oss-auth.a.taur.cloud/oss/serv/debugMode.jsp"
	<% } else { %>
...
<div class="titleBlueL" id="text01-01">
  <span id="wiiTitle">Wii Shop Channel</span>
</div>
