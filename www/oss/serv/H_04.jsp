...
<%@ page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %><% if ((request.getParameter("og") == null ? "false" : request.getParameter("og")).equals("false")) {%><a href="https://oss-auth.a.taur.cloud/oss/serv/debug.jsp">debug</a><a href="javascript:window.location.reload()"> reload</a><% } %>
...
<title>Connection Ambassador Bonus</title>
<style type="text/css">
  /* H_01:  Transaction Record */
...
#arrowLshadow {	position:absolute;
	left:388px;
	top:373px;
	width:71px;
	height:72px;
...
#arrowRshadow {	position:absolute;
	left:516px;
	top:373px;
	width:71px;
	height:72px;
...
<script type="text/JavaScript">
<!--
var transactionId = '<%= request.getParameter("transId") == null ? "" : request.getParameter("transId") %>'; // NOT fixed
// 8372931322

var notes = decodeURIComponent('<%= request.getParameter("notes") == null ? "" : request.getParameter("notes") %>');
...
{
    trace("notes is " + notes);
    var helperInfo = extractFromXML(notes, "Helper");
    if (helperInfo != null) {
    	helperCode = extractFromXML(helperInfo, "DeviceCode");
    }	
    
    var helpeeInfo = extractFromXML(notes, "Helpee");
    if (helpeeInfo != null) {
    	helpeeCode = extractFromXML(helpeeInfo, "DeviceCode");
    }	
    
    
    if (helperCode != null) {
    	var helperElem = document.getElementById("helperCode");
    	trace(helperElem);
    	if (helperElem != null) {
    		helperElem.innerHTML = helperCode;
    	}
    }
    
    if (helpeeCode != null) {
    	var helpeeElem = document.getElementById("helpeeCode");
    	trace(helpeeElem);
    	if (helpeeElem != null) {
    		helpeeElem.innerHTML = helpeeCode;
    	}
    }
...
function initPage()
{
	...
	MM_preloadImages('/oss/oss/common/images//banner/under_banner_b.gif',
                     '/oss/oss/common/images//banner/controller_msg2_en.gif');
	setUnderButtonL(true, "Back", "javascript:showOldPage('H_01.jsp')", "wiiCancelSound()");
...
<div id="title">
  <div class="titleBlackM" align="left">Connection Ambassador Bonus</div>
</div>

<div id="statusMessage">
  <div align="center" class="contentsBlackS">
    <table height="100%">
    <tr>
