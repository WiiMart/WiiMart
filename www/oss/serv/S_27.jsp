...
<title>???S_27.title???</title>
<link href="/oss/oss/common/css/S_27.css" rel="stylesheet" type="text/css" />
...
function initPage() {
    ...
    if(ecSupportsSession()){
		ec.setSessionValue("scaWaitingActivated", null);
    }

	if(goldStatus == 'ACTIVATED') {
		setUnderButton(true, "OK", "javascript:submit()", "wiiSelectSound()");
		showElement("Gold-03");
	} else if(silverStatus == 'ACTIVATED') {
		var isInitialFlow = ec.getSessionValue("S-22fromInit");
		if (isInitialFlow == "true") {
			setUnderButton(true, "OK", "javascript:showPage('W_01.jsp')", "wiiSelectSound()");
		} else {
			setUnderButton(true, "OK", "javascript:submit()", "wiiSelectSound()");
		}
		showElement("Silver-03");
	} else {
		showPage('W_03.jsp');
	}
}
...
<% if (rank != null && rank.equals("CSS_SILVER")) { %>
<div id="Silver-03" class="page_area bg_connect">
	<div class="page_title_area titleBlackM">
	  Connection Ambassador authorization</div>
	<div class="page_content_area contentsBlackM">
	  Now good luck in reaching the goal of helping 20 people!</div> 
</div>
<% } else if (rank != null && rank.equals("SCA_GOLD")) { %>
<div id="Gold-03" class="page_area bg_sca">
	<div class="page_title_area titleBlackM">
	  Connection Ambassador authorization</div>
	<div class="page_content_area contentsBlackM">
	  <br />From here on out, you won&rsquo;t receive any more points for helping others connect. However, those who are helped will receive points. So, please continue to help others!</div> 
</div>


<form id="form" method="post" action="S_18.jsp" style="display: none">
