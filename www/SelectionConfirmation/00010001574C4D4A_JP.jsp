		...
		if (giftStatus == "sending") {
			...
			if (showCtrlPage != "true") {
				if (hasCautionItem2 == "true" || ((genreWifiPay == "true") && (wifiIndicator == WIFI_PARTIALLY_CHARGED || wifiIndicator == WIFI_ALL_CHARGED || wifiIndicator == WIFI_FREE_INPUT))) {
					setUnderButtonR(true, "OK", "javascript:showPage('B_28.jsp?titleId=00010001574C4D4A&itemId=&SD=')", "snd.playSE(cSE_Decide)");
				} else if (hasCautionItem3 == "true") {
					setUnderButtonR(true, "OK", "javascript:showPage('B_09.jsp?titleId=00010001574C4D4A&itemId=&SD=')", "snd.playSE(cSE_Decide)");
				} else {
					setUnderButtonR(true, "OK", "javascript:showPage('B_18.jsp?titleId=00010001574C4D4A&itemId=&SD=')", "snd.playSE(cSE_Decide)");
				}
			}
			...
			if (showCtrlPage == "true") {
				...
				if (hasCautionItem2 == "true" || ((genreWifiPay == "true") && (wifiIndicator == WIFI_PARTIALLY_CHARGED || wifiIndicator == WIFI_ALL_CHARGED || wifiIndicator == WIFI_FREE_INPUT))) {
					...
					setUnderButtonR(true, "OK", "javascript:showPage('B_09.jsp?titleId=00010001574C4D4A&SD=')", "snd.playSE(cSE_Decide)");
				}
			}
...
<div id="TitleName1stline" nowrap>
  <div align="center" class="contentsBlueM">LA-MULANA（ラ・ムラーナ）</div>
</div>
<div id="text02-01" >
  <div align="center" >
...
<div id="text04-02">
    <div align="center" >
      <div align="left" class="contentsBlackS">このゲームは以下のコントローラに対応しております。<BR>・Wiiリモコン<BR>・Wiiリモコン＋ヌンチャク <BR>・Wiiリモコン＋クラシックコントローラ（Pro含む）<BR>・GCコントローラ <BR><BR>※USBキーボードにも対応しています</div>
    </div>
  </div>
</body>
</html>
