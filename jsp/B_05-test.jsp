...
<script>
    ...
    */
    function getUrlParam(param) {
        ...
        if (url.indexOf("&") == -1) {
            ...
            for (var i = 0; i < url.length; i++) {
                var tempUrl = url[i].split("=");
                if (tempUrl[0] == param) {
                    return tempUrl[1];
                    trace("URL Param is: " + tempUrl[1])
                }
            }
        }
    }
    ...
    function getAppDetails() {
        ...
        var platformTxt = document.getElementById("Platform").children[0];
        var publisherTxt = document.getElementById("Publisher").children[0];
        var buyAnchor = document.getElementById("BuyAnchor");
        var photo = document.getElementById("Photo");
        var date = document.getElementById("date").children[0];
        var titleName1 = document.getElementById("TitleName1").children[0];
        var titleName2 = document.createElement("div")
        var genre = document.getElementById("DisplayCategory");
        var players = document.getElementById("Players").children[0];
        for (var i = 0; i < gameslist.length; i++) {
            var titleId = getUrlParam("titleId");
            //trace("titleId::" + titleId);
            var language = getUrlParam("language").toUpperCase();
            //trace("language::"+language)
            if ((gameslist[i].id == titleId) && 
                (gameslist[i].language == language)
               ) {
                titleIndex = i;
                trace("found title: " + gameslist[i].title1 + " in index" + titleIndex + " with language: " + gameslist[i].language);
                if (gameslist[i].title2 !== "") {
                    titleName2.innerHTML = gameslist[i];
                }
                buyAnchor.href = "javascript:onBuyButtonPressed('" + titleId + "', '')";
                publisherTxt.innerHTML = gameslist[i].publisher;
                platformTxt.innerHTML = gameslist[i].console;
                date.innerHTML = "Released " + gameslist[i].date;
                photo.innerHTML = "<img src='/oss/ccs/" + titleId + "/" + gameslist[i].thumbnail + "' width='160' height='120'>";
                titleName1.innerHTML = gameslist[i].title1;
                genre.innerHTML = gameslist[i].genre;
                players.innerHTML = gameslist[i].players
            }
        }
        //platformTxt.innerHTML = s;
...
var titleId = getUrlParam('titleId');
var titleSize = gameslist[titleIndex].size;
var titleTmdSize = '';
...
<!-- ordinary pricing -->
<div id="buyButton">
    <div id="buybanner"><img src="/oss/oss/common/images//banner/buy_a.gif" width="241" height="76" id="Image3" /></div>
